...
### macOS reader UX pass 3 (2026-07-08, from on-device review)

- [x] **Blockquote (paragraph-highlight) comment invisible in Word (both engines)** — reported/fixed 2026-07-10. A `.blockquote` annotation carrying a note wrote its `<w:comment>` into `comments.xml` but Word never showed it: the comment was orphaned with no `commentRangeStart/End/Reference` in `document.xml`. Root cause in `RunPropertyInjector` (Swift + Kotlin): blockquote returns an empty run-property from `rPrForTool`, so the `guard !rPrContent.isEmpty else { continue }` (Swift `:104` / Kotlin `:277`) bailed out **before** the comment-anchor registration, unlike `.bookmark` which is diverted by its own branch earlier. Fix: register the comment anchor for the no-run-property tool before the `continue`, so blockquote+note now emits its range markers. Verified live in Word (comment anchored to the paragraph). Both engine suites green incl. the `RunPropertyInjectorEquivalenceTest` byte-identical guard (change is additive). **Existing files self-heal:** re-open in Layuv + trigger any save (annotations.json is the primary store) to regenerate `document.xml` with markers.
- [x] **Comment author name (all platforms need parity; UI macOS-only so far)** — reported 2026-07-10. Comments were written with `w:author = annotation.id` (a bare number, e.g. `1783711920430118`) because Layuv had no name. Added an optional `author` param to `DocxStore.write` → `CommentWriter.buildCommentsXml`/`buildNoteComment` in **both** engines; null falls back to the id (goldens stay byte-identical). App side (**macOS only**): new `AppSettings` singleton (`author_name`, UserDefaults) with a **Settings scene (⌘,)** "Author Name" field (global default, falls back to "Layuv"); **per-document override** via `DocumentStore.currentDocAuthor` (persisted in a `docAuthors` path→name map, mirroring the `docThemes` pattern) exposed as Format → "Author for This Document…" (AppKit `NSAlert` prompt). Effective author = per-doc override → global name → "Layuv"; threaded into `save()` + `finishInk()`. **PARITY TODO (iOS/iPad + Android):** the engine change is done on both, but only macOS has the author-name UI. iOS/iPad need the global field (their settings surface is the AI-settings sheet — `iOS/AiSettingsView.swift`) and optionally a per-doc override; Android needs an author-name setting wired into `DocxStore.write(..., author=)`.
- [ ] **Cross-platform parity sweep (tracking)** — bring iOS/iPad + Android up to macOS feature parity for everything landed in these macOS passes (author-name UI, blockquote-in-tool-picker, delete-from-tap, side-panel jump, etc.). **EXCEPTION: multi-window is macOS-only** and stays that way until iPadOS supports the equivalent multi-scene/document model. Track per-feature parity notes inline above.
- [x] **Reader line-spacing parity + setting (macOS/iOS)** — added `LineSpacing` enum (Normal 1.15 / Comfortable 1.25 / Spacious 1.32, mirroring Android `ReaderTheme.lineSpacingMult`), applied as a paragraph-style `lineHeightMultiple` on the reader's base attributes and threaded through both readers (`lineHeight` param on `makeAttributedString` + `update`) so it re-paginates live. Persisted in `DocumentStore.lineSpacing` (default Comfortable). Picker added to the Format menu + Typography toolbar menu (macOS) and the Aa reader menus (iOS/iPad). Commit `c278883`.
- [x] **Sandbox-safe atomic save (macOS)** — annotation writes silently failed with `NSCocoaErrorDomain 513 "Operation not permitted"` when the DOCX lived in iCloud Drive or any user-selected folder: a file-scoped security bookmark grants read/write on the file but NOT permission to create a sibling `.tmp` in its parent dir. Fixed by staging the temp in `FileManager`'s `.itemReplacementDirectory` (same volume, sandbox-permitted) then `replaceItemAt`. Verified live: `comments.xml` now lands in the file. Commit `2d6002f`.
- [ ] **Delete annotation from tap (macOS)** — tapping an annotation in the body opens the edit sheet but offers no Delete action; there's currently no way to remove a (non-ink) annotation by clicking it. Add a Delete affordance to the annotation edit sheet / tap flow (`AnnotationEditSheet` + `DocumentStore.deleteAnnotation` already exists — just needs wiring in the UI).
- [ ] **Side-panel annotation → jump to body (macOS)** — tapping an annotation row in the Annotations side panel does NOT scroll the reader to that annotation; it just surfaces the panel. Wire the row tap to scroll/jump the reader to the annotation's location (macOS has `ReaderCoordinator.scrollToCharOffset` / `scrollToAnnotation` plumbing from the nav-pane work — reuse it).
- [ ] **DOCX write bloat (both engines?)** — first save inflated the file ~10× (45 KB → 495 KB, stable thereafter), likely `DocxStore.write` re-zipping stored/uncompressed. Round-trips fine but bloats iCloud. Investigate compression on write.
- [x] **Second window/instance opens the SAME document as the first (macOS)** — reported 2026-07-10, FIXED 2026-07-10 (multi-window refactor, builds clean; on-device verify pending). The real fix: `LeamhAppApp.swift` no longer shares one `@StateObject DocumentStore` across all windows. Now a **value-based `WindowGroup(for: URL.self)`** where each window (`HomeWindow`) owns its own `@StateObject DocumentStore` and loads its keyed URL via `.task(id: url)`. File-open requests open a NEW window instead of mutating a shared store: Finder/`open` routes through the app delegate's captured `OpenWindowAction` (`open urls:` → `openWindow(value:)`); `Open…` (menu + in-window) and the sidebar open button call `DocumentStore.runOpenPanel()` (new static picker that returns a URL without loading) → `openWindow(value:)`. Menu-bar File/Format commands act on the focused window via `@FocusedObject` (`.focusedSceneObject(store)`) instead of a global store. `WindowGroup(for:)` dedupes by URL value, so re-opening a shown file just focuses its existing window. iPad/iPhone/Supernote untouched (macOS-only per request). Empty-state recents still load into the current blank window (no stray window). — Prior investigation (all still valid): (1) Debug+Release builds coexisting under one bundle ID confused Launch Services → fixed with `macos_native/build_and_install.sh` (single canonical `/Applications/Layuv.app`, deletes its own DerivedData copy after install, refuses to install on an uncommitted tree — commits `c730002`, `f6f83e4`, `4995f80`); (2) `LSMultipleInstancesProhibited` wasn't reaching the built Info.plist because `INFOPLIST_KEY_LSMultipleInstancesProhibited` in `project.pbxproj` doesn't merge with this target's custom `INFOPLIST_FILE` — fixed by setting the key directly in `Info-DocumentTypes.plist` (commit `2ac277b`), confirmed `true` in the built Info.plist, confirmed `open -n` no longer spawns a second **process**. BUT the underlying bug remains: opening a different document while Layuv is already running still loads it into the *existing* window/instance showing the OLD document instead of opening it as a new window — i.e. this was never actually a duplicate-process bug, it's `LeamhAppApp.swift`'s `application(_:open:)`/`onOpenURL` handling (routes into the single `DocumentStore`) failing to open a second window for a second file. Next step: read `macos_native/LeamhApp/LeamhApp/macOS/LeamhAppApp.swift` and `DocumentStore.openAny` — likely needs genuine multi-window support (`WindowGroup` keyed per open document, or an `NSDocumentController`-style per-file window lookup) rather than one `DocumentStore` driving one `WindowGroup` scene.
...
- [ ] **Export annotations with anchors, no full-document read needed (scoped 2026-07-09)** — Export for AI (Android + Swift, both already package chapter text + annotations as Markdown) should also emit each annotation's anchor (position fraction + anchored text/context — whatever `DocxStore`/engine already tracks for on-page anchoring) alongside its note/tag/tool, so an AI reading the export can match an annotation to its spot in the manuscript without ingesting the whole document. Include the **source filename** in the export (currently the export folder/markdown may not surface which document it came from — needed once a user has multiple exports around). Engine-side: anchors already exist for in-app annotation placement (`RunPropertyInjector`/bookmark IDs on Android, engine-JSON pos on Swift) — check whether the exporter already has access to that data or needs to thread it through; likely no new engine work, just surfacing existing fields in the export writer (`ExportForAi`-equivalent on both platforms).
...
- [ ] **Thread-editor UI parity (macOS/iOS Swift, deferred — scoped 2026-07-09)** — app-layer only; engine (above) is ready, nothing blocking. Android's `NoteActivity` has a full Thread tab: reply list (text + timestamp + "· Word" tag for imported entries), Reply/Edit/Delete per entry (Edit/Delete hidden for Word-imported entries — read-only), compose field that flips Add/Update. Swift's `Shared/AnnotationEditSheet.swift` only edits a single note/tag/tool today — no reply list, no add-reply UI.
  - **Plan:** add 3 thin `DocumentStore` methods (`addThreadReply`, `updateThreadEntry`, `deleteThreadEntry`) that build a new `Annotation` via `.copy(threadEntries:)` and route through the *existing* `updateAnnotation()` → `save()` path — no new write path. Add a Note/Thread tab picker to `AnnotationEditSheet.swift` (Shared/, single implementation for both platforms per existing `#if os(macOS)/#if os(iOS)` convention — only sizing/layout differs). Verify entry-0/`note` back-compat (entry 0 mirrors `note`) against `CommentWriter.swift` before wiring delete, so deleting the first entry doesn't orphan `note`.
...
- [ ] **Paged-mode two-column pagination + cross-column selection (macOS Swift, deferred — medium/large rework, 2026-07-09 scoping)** — two related bugs, same root cause. (1) Occasional missing/misplaced line at the column-1→column-2 seam: `ReaderView.swift` measures page/column line breaks with one throwaway `NSLayoutManager`/`NSTextContainer` pass (`paginate()`, ~line 809-832), then hands the resulting ranges to *separate*, persistent per-column `NSTextView`s (`relayoutPaged()`/`makeTextView()`) that re-lay-out the substring themselves — two independent layout passes aren't guaranteed to agree on line breaks, so a boundary can shift and clip/duplicate a line. (2) No drag-selection across the column gap: each column is a fully separate `NSTextView` (`columns: [AnnotatingTextView]`, line 173) with independent selection state; AppKit has no built-in way for one drag to span two sibling text views. Also affects Find (current-page-only in paged modes).
  - **Fix shape:** adopt TextKit 2's `NSTextContentStorage` shared across multiple `NSTextContainer`s (one per column, one shared `NSTextLayoutManager`) — lay out once instead of twice, which fixes the line-boundary bug and makes a unified cross-container selection range tractable (though the visual cross-container selection UI would still need custom rendering). Touches `columns`, `makeTextView()`, `relayoutPaged()`, `paginate()`, selection-change handling, and Find. Estimated medium-to-large refactor, not a tactical patch.
  - **Android precedent:** `android_native`'s `Paginator.kt` already does this right — one `StaticLayout` for the whole document, sliced into columns purely by clip+translate at draw time (explicit comment guarantees nothing is ever clipped mid-line). That's the architecture to mirror on Swift. (Android's selection is tap/drag-over-canvas, not native text-view selection, so it doesn't directly prove cross-column *selection* — only the single-layout approach is validated precedent.) User confirmed hitting the same missing-line issue on Android historically.
