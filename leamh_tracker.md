...
- [ ] `ThreadEntry` parity — `ManuscriptSerializer` is note-only; full thread-folding round-trip with Android-threaded annotations still pending. (Port spec fully scoped 2026-06-27: add `ThreadEntry` struct + `Annotation.threadEntries`; reply-flatten via `commentsExtended.xml` in `LegacyComments`; `CommentWriter.buildNoteComment` per-entry paragraphs + `emptyCommentsExtended`; `Timestamps.formatThreadPrefix`; fold thread in `ManuscriptSerializer.noteText`; mirror Android fixtures `golden/import/thread.*`.)
