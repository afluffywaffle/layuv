...
class PageFlipReaderState extends State<PageFlipReader> {
  ...
  static const double _counterZone = 64.0;
  ...
  List<_Page> _paginate(String text, double maxWidth, double maxHeight) {
    ...
    // Estimate chars-per-page from a bounded sample so the main loop only
    // lays out a small window per page rather than the entire remaining text.
    // Sample the first 2000 chars to get a chars-per-line figure, then use
    // ~2× that estimate as the window fed to TextPainter each iteration.
    // This makes pagination O(n) instead of O(n²).
    const sampleLen = 2000;
    final sample = text.length > sampleLen ? text.substring(0, sampleLen) : text;
    final samplePainter = TextPainter(
      text: TextSpan(text: sample, style: kReaderTextStyle),
      textDirection: TextDirection.ltr,
    )..layout(maxWidth: colWidth);
    final sampleLines = (samplePainter.height / lineHeight).clamp(1, double.infinity);
    final charsPerLine = sampleLen / sampleLines;
    final linesPerPage = (safeHeight / lineHeight).clamp(1, double.infinity);
    // Window = 2× estimated page content, so the cut point is always inside it.
    final windowChars = ((charsPerLine * linesPerPage) * 2).ceil().clamp(500, 80000);

    final pages = <_Page>[];
    int start = 0;

    while (start < text.length) {
      final windowEnd = (start + windowChars).clamp(0, text.length);
      final window = text.substring(start, windowEnd);

      final painter = TextPainter(
        text: TextSpan(text: window, style: kReaderTextStyle),
        textDirection: TextDirection.ltr,
      )..layout(maxWidth: colWidth);

      if (painter.height <= maxHeight) {
        // The window fits: either it's the last chunk or the window was too small.
        // If we haven't reached the end of the document, widen and retry once.
        if (windowEnd < text.length) {
          final fullChunk = text.substring(start, (start + windowChars * 2).clamp(0, text.length));
          final p2 = TextPainter(
            text: TextSpan(text: fullChunk, style: kReaderTextStyle),
            textDirection: TextDirection.ltr,
          )..layout(maxWidth: colWidth);
          if (p2.height <= maxHeight) {
            // Still fits — this is the last page.
            pages.add((text: fullChunk, offset: start));
            break;
          }
          // Doesn't fit in doubled window: fall through with original window as page.
        }
        pages.add((text: window, offset: start));
        start += window.length;
      ...
      pages.add((
        text: window.substring(0, end).trimRight(),
        offset: start,
      ));
      start += end;
      while (start < text.length && text[start] == '\n') {
