...
class DocxStore implements AnnotationStoreInterface {
  ...
  Archive _injectNativeFormatting(
    ...
    for (final a in annotations) {
      // Rebuild map + run lists every iteration — xml may have grown from splits.
      final map = _buildPlainMap(xml);
      if (map.plain.isEmpty) continue;

      final loc = _locateInPlain(
          map.plain, a.selectedText, a.prefix, a.suffix,
      ...
      final startXmlPos = map.xmlOffsets[loc.start];
      final endXmlPos = map.xmlOffsets[loc.end - 1];
